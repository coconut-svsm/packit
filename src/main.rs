...
use packit::{PackItArchiveEncoder, PackItArchiveDecoder, PackItError, PackItFile, PackItResult};

#[macro_export]
macro_rules! ioerr {
    ($k:ident) => { io::Error::from(ErrorKind::$k) };
...
impl PackParams {
	...
	fn process_entry<W: Write>(
		...
		if etype.is_file() || (etype.is_symlink() && !self.no_symlinks) {
			...
			let dst_path = path.strip_prefix(&self.input)
